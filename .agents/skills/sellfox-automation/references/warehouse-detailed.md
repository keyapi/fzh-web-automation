...
> 最后更新: 2026-05-15
> 探索状态: **全部完成** — 全部已知未知已验证，API 批量自动化已实现，warehouseIds 参数已明确。
...
### 仓库选择器
- **组件类型**：自定义多选组件，`select-container` + `select-dropdown__item`
- **可选仓库**（API 获取到 4 个）：
  | ID | 名称 | 类型 | 库存 |
  |----|------|------|------|
  | 279814 | CENTRADE | type=3 | ~449 |
  | 279833 | DANEEY | type=3 | ~818 |
  | 279841 | POLAND | type=3 | ~1014 |
  | 272150 | 虚拟仓库 | type=-1 | 0 (空仓) |
- **注**：赛狐仓库名与通途完全不同。通途的 6 仓库在赛狐中对应 3 个真实仓库
- **交互**：全选/多选 + 按住 Shift 快速多选 + 取消/确定按钮
- **默认状态**：全部仓库选中（placeholder="全部仓库"）
- **warehouseIds API**：`""`=全部, `"279814,279833"`=多仓, `"279814"`=单仓，已验证 449+818+1014=2281

### 隐藏0数据记录
- **位置**：工具栏，`span:text-is("隐藏0数据记录")`，top:259
- **默认状态**：勾选（隐藏 0 库存行）
- **⚠️ 持久化陷阱**：**每次新 session 打开页面，此选项重置为 ON（1494 条）**！
  必须每次导出前检查并切换。`sellfox_auto_export.py` 已内置检测。
...
#### 搜索类型
- **下拉选择器**：`input[placeholder="请选择"]` (left:28, top:182)，展开后 8 个选项
- **选项**：SKU（默认）/ 识别码 / 品名 / 型号 / FNSKU / SPU / 款名 / MSKU
- **切换方法**：click el-select wrapper → 选 item

#### 搜索模式（精/模）
- **切换按钮**：`.search_type_btn` (left:267) — 点 icon 无效，必须点外层容器
- **图标**：`.icon_sf_fuzzy`（模糊/模） / `.icon_sf_precise`（精确/精）
- **提示 popover**："这里可切换精确/模糊搜索"

#### 搜索输入框
- **placeholder 动态变化**："搜索内容" ↔ "双击可批量搜索内容"
- **触发方式**：输入后按 Enter 搜索
- **批量搜索**：`.icon_sf_multiline`（多行批量搜索）

#### 实测数据

| 类型 | 模式 | 关键词 | 结果 | 说明 |
|------|------|--------|------|------|
| SKU | 模糊 | KS0001 | 801条 | 匹配所有 KS0001-xxx-xxx |
| SKU | 精确 | KS0001 | 0条 | 不存在恰好叫 KS0001 的 SKU |
| SKU | 精确 | KS0001-DM-60-GREYPURPLE | 1条 | 完整 SKU 精确命中 |
| 品名 | 模糊 | 三角靠枕 | 970条 | 品名含"三角靠枕"的全部SKU |

#### 踩坑
- 搜索输入框 placeholder 会动态切换（"搜索内容"/"双击可批量搜索内容"），不能用固定选择器
- 精/模切换不能直接点 icon（会被 popover 拦截），必须点 `.search_type_btn` 外层容器
- 页面有其他展开的下拉菜单时会阻止搜索栏交互，需先关闭

### 分页
- **组件**：`el-pagination`
- **每页条数**：20（默认）/ 50 / 100 / 200 条/页
- **导航**：页码按钮（1-7 直接显示）+ 省略号 + 尾页 + 前往指定页输入框
- **总数显示**："共 XXXX 条"
- **已验证**：点击第 2 页正常跳转，2281 条 = 115 页

### 重置按钮
- **位置**：搜索栏右侧 `button:has-text("重置")` (left:336)
- **行为**：清除所有过滤条件和搜索内容
- **注意**：需验证"隐藏0数据记录"是否被重置带回默认

### 搜索输入框选择器（重要！）
- **placeholder 会变**：输入前="双击可批量搜索内容"，有值时="搜索内容"
- **推荐选择器**：按位置定位 `input` (left:107, top:182)，不用 placeholder
- **稳健方法**：`browser_evaluate` 找到后直接用 JS click
...
### 坑 6：下拉菜单未关闭遮挡后续操作
- **现象**：打开仓库下拉后，搜索栏、精/模按钮全部点不动（"subtree intercepts pointer events"）
- **原因**：页面上 el-popper 弹窗仍然展开，遮挡了后面的元素
- **解决**：每次操作完下拉菜单后，点一下页面标题区域关闭弹窗，或按 Escape
- **教训**：操作新的页面区域前，先检查 `anyDropdownOpen` 状态
...
- [x] 全选/取消全选行为
- [x] 任务 ID 来源
- [x] API 自动化方案 → Python 脚本已实现 (--api 模式)
- [x] 仓库列表 → 4个(含虚拟仓库272150) + 导入仓库id: CENTRADE=279814, DANEEY=279833, POLAND=279841
- [x] 隐藏0数据影响 → 1494→2281 行
- [x] 搜索功能 → 8 种搜索类型 + 精确/模糊
- [x] 分页 → 20/50/100/200 条/页
- [x] **仓库切换** → API 验证：单仓449+818+1014=2281=全部 ✅
- [x] **分页导出范围** → 导出的是全部数据，不是当前页
- [x] **过滤条件跨会话持久化** → **不持久化！** 隐藏0数据每次重置为ON (1494条)
  - 这是自动化的重要陷阱：每次打开页面都必须检查并切换隐藏0数据！
- [x] **sf-vvv-i token** → 由前端 JS 动态生成（基于 sf_u cookie），不需要跨会话复用
- [x] **cookie+API** → sf_u 跨会话不变，已通过 --api 模式验证可直接调 API
- [x] **warehouseIds 参数** → 空字符串=全部，逗号分隔=多仓，单个ID=单仓
  - 全选 `""` = CENTRADE(449) + DANEEY(818) + POLAND(1014) = 2281

## 仓库清单（API 备份）

```python
WAREHOUSES = [
    {"id": "279814", "name": "CENTRADE"},
    {"id": "279833", "name": "DANEEY"},
    {"id": "279841", "name": "POLAND"},
    {"id": "272150", "name": "虚拟仓库", "type": -1},  # 空仓，忽略
]
```

## 后续开发方向

1. ✅ **Python 脚本**：`sellfox_auto_export.py` 已实现，浏览器+API 双模式
2. ✅ **API 逆向**：全链路已打通（export → pageList → download → COS URL）
3. ~~**数据合并**~~：无需——选全部仓库导出就是一个文件（含所有仓库数据）
4. ⬜ **多仓库批量**：可分别导各仓库 + 对比（用 warehouseIds 参数）
